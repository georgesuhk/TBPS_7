...
if __name__ == '__main__':
    thresholds = [0.995, 0.99, 0.95, 0.90, 0.85, 0.80, 0.75, 0.50, 0.25,
                  0.2, 0.15, 0.10, 0.05, 0.01, 0.005]
    experiment_files = ["total_dataset.pkl", "acceptance_mc.pkl"]
    for experiment_file in experiment_files:
        for threshold in thresholds:
        # NOTE: only change the values of variables in CAPS when using!
        # setup
            EXPERIMENT = experiment_file
            COMBINATORIAL_THRESHOLD = threshold
            ...
            plt.xlabel("B0 Mass")
            plt.ylabel("Number of Events")
            masses_filtered = df_comb.query("label_peaking < 3")["B0_M"]
            print("Number of total combinatorial background events:",
                  len(masses_filtered))
            plt.hist(masses_filtered, bins=int(1e2), label="Combinatorial Background "
                                                           "Events")
            plt.legend()
